...
Two steps: ``techo timeline`` generates the per-page edition PDF, and
``techo timeline-merge --size ...`` assembles the print spread from it.
"""

from .. import build, sizes

MAX_HOUR = 99
LABEL_PT = 10.2  # pt ≈ 3.6 mm (mini-tool label size)
...
def generate(
    ...
    *,
    ...
    print(
        ...
        f"merge with `techo timeline-merge --size {size}`"
    )


def merge(size: str, side: str = "binding") -> None:
    """Assemble ``spread.pdf`` from the pages generated by ``techo timeline``.

    ``side="binding"`` places the even page (axis on its right) on the left
    half and the odd page (axis on its left) on the right, so both axes hug
    the binding with the labels between the axis and the spine; ``side="outer"``
    keeps natural page order (axes on the outer edges).  Booklet sizes
    (tn/tnp) always bind at the center regardless of ``side``.
    """
    s = sizes.SIZES[size]
    PW, PH = s["pw"], s["ph"]
    edition = f"timeline-{size}"
    out = build.OUTPUTS / edition
    if not (out / f"{edition}.pdf").exists():
        raise RuntimeError(
            f"no generated edition at {out}; run `techo timeline --size {size}` first"
        )
    build.write_spread(out, edition, PW, PH, booklet=size in ("tn", "tnp"), side=side)
    print(f"Generated {out}/spread.pdf (spread, {PW * 2}×{PH}mm, {side} side)")
